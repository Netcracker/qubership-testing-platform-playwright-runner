...
function runHttpTest() {
  return new Promise((resolve) => {
    const server = http.createServer((req, res) => {
      ...
      // 6. trace-id present
      if (headers['trace-id']) {
        pass(`"trace-id" header present: ${headers['trace-id']}`);
      } else {
        fail('"trace-id" header is MISSING');
        failures++;
      }

      // 7. span-id present
      if (headers['span-id']) {
        pass(`"span-id" header present: ${headers['span-id']}`);
      } else {
        fail('"span-id" header is MISSING');
        failures++;
      }

      // 8. x-b3-sampled present
