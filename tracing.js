...
const { propagation, trace, ROOT_CONTEXT, defaultTextMapGetter } = require('@opentelemetry/api');

const { TraceIdSpanIdPropagator } = require('./lib/trace-id-span-id-propagator');
...
// Patch console.log so that Node.js test output carries the active OTel trace ID,
// matching the [traceId=...] prefix that bash logging.sh adds to shell-level logs.
const _origLog = console.log;
console.log = (...args) => {
  const spanContext = trace.getActiveSpan()?.spanContext();
  const traceId = spanContext?.traceId;
  if (traceId) {
    _origLog(`[traceId=${traceId}]`, ...args);
  } else {
    _origLog(...args);
  }
};
